...
// Add new delivery
app.post('/api/sheets/delivery', async (req, res) => {
  try {
...
// Update delivery
app.put('/api/sheets/delivery/:id', async (req, res) => {
  try {
    const { id } = req.params;
...
async function updateStockAndSales(delivery) {
  try {
    ...
    for (const product of delivery.products) {
      ...
      if (stockIndex > 0) {
        ...
        await sheets.spreadsheets.values.update({
          ...
          valueInputOption: 'RAW',
          requestBody: { values: [[newQuantity, timestamp]] },
        });
      }
    }
  } catch (error) {
